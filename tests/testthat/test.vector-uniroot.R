...
test_that("vector_uniroot preserves independently bracketed decreasing roots", {
  roots <- c(1, 2, 3)
  solved <- HeatStress:::vector_uniroot(
...
test_that("vector_uniroot expands the required side of decreasing brackets", {
  ...
  solved <- HeatStress:::vector_uniroot(residual, c(0, 0), c(2, 2),
...
test_that("vector_uniroot isolates mixed row failures", {
  roots <- c(1, 5, -5, 20, 1)
  solved <- HeatStress:::vector_uniroot(function(x, idx) {
    value <- roots[idx] - x
...
test_that("vector_uniroot returns initial endpoint roots directly", {
  roots <- c(0, 2)
  solved <- HeatStress:::vector_uniroot(function(x, idx) roots[idx] - x,
...
test_that("vector_uniroot reports maximum iteration failures", {
  solved <- HeatStress:::vector_uniroot(function(x, idx) 2 - x ^ 2,
...
test_that("vector_uniroot recovers from a non-finite secant candidate", {
  ...
  solved <- HeatStress:::vector_uniroot(residual, 0, 2, 0, 2, tolerance = 1e-8)
...
test_that("vector_uniroot fails safely when secant and midpoint are non-finite", {
  ...
  solved <- HeatStress:::vector_uniroot(residual, 0, 2, 0, 2)
  expect_false(solved$converged)
...
test_that("vector_uniroot supports coherent zero-length inputs", {
  solved <- HeatStress:::vector_uniroot(function(x, idx) numeric(), numeric(),
...
test_that("vector_uniroot rejects invalid controls", {
  ...
  expect_error(do.call(HeatStress:::vector_uniroot, c(args, list(tolerance = 0))),
    "finite positive")
  expect_error(do.call(HeatStress:::vector_uniroot, c(args, list(tolerance = Inf))),
    "finite positive")
  expect_error(do.call(HeatStress:::vector_uniroot, c(args, list(max_iterations = 0))),
    "positive integer")
  expect_error(do.call(HeatStress:::vector_uniroot, c(args, list(max_iterations = 1.5))),
    "positive integer")
  expect_error(HeatStress:::vector_uniroot(root, c(0, 0), 2, c(-1, -1), c(3, 3)),
    "same length")
  expect_error(HeatStress:::vector_uniroot(root, 2, 0, -1, 3), "lower must")
  expect_error(HeatStress:::vector_uniroot(root, 0, 2, 1, 3), "lower_limit")
  expect_error(HeatStress:::vector_uniroot(root, 0, 2, -1, 1), "upper_limit")
})
